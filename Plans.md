...
|------|------|-----|---------|--------|
| 2.1 | コンパニオン スタイル基盤（hisho 踏襲）: g2hermes に `tailwindcss` + `@tailwindcss/vite`（devDeps）追加、`vite.config.ts` に `@tailwindcss/vite` プラグイン、`src/app.css` 新設（`@import "tailwindcss"` + toolkit `theme-light`/`typography`/`utilities` + `@source` 走査 + `@theme` + `#root max-width:430px`）、`main.tsx` で `import './app.css'`。**別の UI フレームワークは入れない**。`app.json`・network whitelist は不変 [tdd:skip:scaffold-config] | `bun run --filter g2hermes build` 成功、`biome check` 0。`App.tsx` に toolkit コンポーネントを 1 個仮置きしてシミュレーターで hisho 同等スタイル（utilities が効く）を目視。`app.json` 差分なし | - | cc:完了 [0045cba] ／ tailwindcss + @tailwindcss/vite を devDeps 追加（既に root hoist 済・bun.lock に依存エッジ記録）。vite.config に plugin、src/app.css を hisho 踏襲で新設（@import tailwindcss + toolkit theme-light/typography/utilities + @source 走査 + @theme 26 vars + #root max-width:430px + scrollbar-hide）。main.tsx で import。biome.json に css.parser.tailwindDirectives 追加（app.css の at-rules 対応・hisho 同設定）。App.tsx に ScreenHeader を仮置き（utilities 検証用、2.5 で Companion に置換）。検証: build 成功（CSS 32.38 kB=utilities 生成確認）・biome 0・bun test 53 pass・`git diff app.json` 空。別 UI FW 未導入。Codex review（正規 mjs --base 6296ca4）APPROVE |
...
| 2.3 | `companion/storage.ts`（hisho 参考・配列対応分を拡張）を **TDD**: `bridge.setLocalStorage/getLocalStorage` 正本 + `waitForEvenAppBridge` 1500ms タイムアウトで dev は browser localStorage フォールバック（hisho 踏襲）+ 書き込み直列化キュー（hisho 踏襲）。キー `g2hermes.presets` に presets 配列を **JSON 文字列**で保存（全件 1 キー上書きで削除表現）。`loadPresets()`（同期プリロード用に検証通過配列 or default を返す）/ `savePresets()` [tdd:required]（bridge をモック注入） | `bun test` グリーン: bridge 有→set/get が JSON 配列を round-trip / bridge タイムアウト→localStorage フォールバックに切替 / 連続 save が直列化順序を保つ / get 失敗時 default へ。`biome check` 0 | 2.2 | cc:完了 [4582ae8]（**TDD**。`companion/storage.ts`＝`createPresetStore(deps)` ファクトリで `PresetBridge`/`PresetLocalStorage` を注入可能化〔`getBridge:()=>Promise<PresetBridge\|null>` + 任意 `localStorage`〕。`loadPresets`=bridge or localStorage の raw を `presets.parse()` に委譲〔検証/default フォールバックは 2.2 の parse 再利用＝重複なし〕、catch→`parse('')`。`savePresets`=`serialize()`→直列化キュー〔hisho 踏襲〕で全件 1 キー上書き〔削除＝件数減も反映〕・失敗握り潰し。production は `tryGetBridge`〔`waitForEvenAppBridge`+1500ms race・bridgePromise 非破棄〕注入の singleton を `loadPresets`/`savePresets` wrapper で export。実 SDK `setLocalStorage` は `Promise<boolean>` のため interface 戻り値は `Promise<unknown>`。RED→GREEN: storage 9 pass〔round-trip・全件上書き・未設定/不正JSON/get throw→default・localStorage fallback round-trip・localStorage 未設定→default・直列化順序〔delay 30/20/10 で逆転検知〕・save 失敗が後続キュー非破壊〕、全体 53 pass・`biome check` 0・`bun run build` 成功。**Codex review 正規ルート**（公式 openai-codex v1.0.4 / `--base 2adcce2 --scope branch`）= **[P2] 1 件のみ→APPROVE**〔bridge 初期化 >1500ms の実機で初回 read が SDK 保存値を読み逃す可能性。spec §4/§7 が許容した hisho 踏襲トレードオフで仕様矛盾でなく、保存は bridge 遅延 resolve 後 SDK へ収束。**恒久対策（bridge resolve 後の再読込→setState）は住処が 2.4 lift-up のため follow-up 化**。2.3 に subscribe API 追加は YAGNI〕） |
| 2.4 | 状態 lift-up（**外部ストア無し**）: 会話 `useReducer` と presets を `AppGlasses.tsx` → `App.tsx` へ持ち上げ、`App` が `state`/`dispatch`/`presets` を `AppGlasses` に props で渡す。`AppGlasses` は受け取った値から `snapshotRef` を組む（`useGlasses` 100ms ポーリングは無改変）。起動時 `App` で `storage.loadPresets()` → presets 初期値。`screen.ts` idle は presets を `PRESETS` 定数でなく props 由来で列挙（「🎤 話す」と併存）[tdd:skip:integration-simulator] | 既存 `glass/reducer.test.ts` が無改変で green を維持。`bun run build` 成功・`biome check` 0。シミュレーターで現行フロー（idle↕・送信→Thinking→回答・音声メニュー）が回帰なし、かつ保存済み presets が idle に出る（実機はユーザー） | 2.2, 2.3 | cc:完了 [430b8ce]（**外部ストア無し lift-up**。`App.tsx`＝`useReducer(reduce,initialState)` と `useState<Preset[]>(DEFAULT_PRESETS)` を保持し `state`/`dispatch`/`presets` を `AppGlasses` に props 渡し。起動時 `useEffect`→`loadPresets()`→`setPresets`（未保存/不正は loadPresets が DEFAULT_PRESETS 返却＝idle 空フラッシュ無し）。`AppGlasses.tsx`＝ローカル `useReducer` と `PRESETS` 定数を撤去し props 受け取りに変更、`snapshotRef.current={...state,presets}`（`useGlasses` 100ms ポーリング無改変）。`dispatch` を prop 化したため exhaustive-deps で startMic/runAsk/handleStop/beginRecording/back/nextPage/prevPage の依存配列に `dispatch` 追加（useReducer dispatch は安定＝再生成なし）。**`screen.ts` は既に `snapshot.presets` を列挙＝無改変**、`reducer.ts`/`reducer.test.ts` も無改変。型は App が `Preset[]`（id 付き・2.5 編集 UI 共有用）保持、glass 層は `PresetQuestion[]`（label/text）で受け疎結合維持（`Preset[]` は構造的に代入可）。検証: `bun test` 53 pass〔reducer.test.ts 無改変 green〕・`biome check` 0・`bun run build` 成功。**Codex review 正規ルート**（公式 openai-codex v1.0.4 / `--base f4b2834 --scope branch`）= **指摘なし→APPROVE**。**Codex P2 follow-up（bridge 初期化 >1500ms で初回 loadPresets が SDK 保存値を読み逃す）への対応**: 本タスクの起動時 `loadPresets` は one-shot のため未解決＝保存値の確実反映は Companion 編集（2.5）の write-through で state がキャッシュを兼ねる前提。恒久対策（bridge 遅延 resolve 後の再読込→setState）は実機で >1500ms が観測された場合に追加検討する follow-up として継続〔hisho 同パターンが実機稼働実績・YAGNI で先送り〕。シミュレーター/実機の回帰確認はユーザー） |
| 2.5 | A 保存プリセット編集 UI（`even-toolkit/web`・hisho 踏襲）: `companion/Companion.tsx`（`AppShell`+`NavHeader`+`ScreenHeader`/`SectionHeader`）と `companion/PresetEditor.tsx`（リスト=`Card`+`ListItem`〔`onDelete` スワイプ削除〕、ラベル=`Input`、プロンプト=`Textarea`、追加/並べ替え up/down=`Button`）。編集→`setPresets`→`storage.savePresets`（write-through）。`App.tsx` が `<Companion/>` を描画（`<AppGlasses/>` 据え置き）[tdd:required]（編集ハンドラ＝追加/更新/削除/並べ替え/検証反映 をピュア関数化して unit） | `bun test` グリーン（編集ハンドラ unit: 追加・更新・削除・並べ替え・検証 reject）。`bun run build` 成功・`biome check` 0。シミュレーターでスマホ UI に CRUD+並べ替えが表示・操作でき、保存が idle へ反映（実機の再起動永続はユーザー） | 2.1, 2.4 | cc:完了 [2bbfd27]（**TDD**。`companion/editor.ts`＝編集ハンドラを純粋関数化〔`addPreset`(上限ガード)/`updatePreset`(部分更新・編集途中の不正値も保持)/`removePreset`(下限ガード=最後の1件は消さない)/`movePreset`(端・範囲外は不変)/`canPersist`(件数 [MIN,MAX]+全件 validatePreset＝parse 受け入れ条件と一致し不正リストの保存を拒否)〕、全て immutable。RED→GREEN: `editor.test.ts` 23 ケース〔追加/更新/削除/並べ替え/検証 reject + 各 immutable + 端/上限/下限〕。`PresetEditor.tsx`＝`Card`+`ListItem`〔`onDelete` スワイプ削除・下限時は onDelete 無効化〕+`Input`(ラベル)+`Textarea`(プロンプト)+up/down/追加 `Button`〔端で disabled・上限で追加 disabled〕、検証落ち項目に未保存ヒント。`Companion.tsx`＝`AppShell`+`NavHeader`+`ScreenHeader`+`SectionHeader`、保存保留ヒント。`App.tsx`＝2.1 の `ScreenHeader` 仮置きを `<Companion/>` に置換し `handlePresetsChange` で write-through〔`canPersist` 時のみ `savePresets`〕、`<AppGlasses/>` 据え置き。**Codex review 正規ルート**（公式 openai-codex v1.0.4 / `--base 5a47b60 --scope branch`）初回 P2×2 を即修正→再レビュー APPROVE: (1) `dirtyRef` で起動時 `loadPresets` 解決が編集中 state を上書きしない〔bridge 1500ms 待ち / 実機初期化遅延での編集ロスト防止〕、(2) グラスへは `validatePreset` で絞った valid 部分集合のみ渡し idle 空行/空 text 送信を防ぐ〔editor は draft 全件・glass は valid のみに分離〕。検証: `bun test` 76 pass・`biome check` 0・`bun run --filter g2hermes build` 成功〔CSS 32.61 kB〕・`git diff app.json` 空。シミュレーターでの CRUD+並べ替え目視・実機の再起動永続はユーザー） |
| 2.6 | B その場送信 UI + 既存 ask 共有: `companion/AskBox.tsx`（`Textarea`+送信 `Button`）→ `App` の ask 起動（reducer に `ASK` dispatch、label=`(カスタム)`）を呼ぶ。回答は同一 `state` を購読し `Card` で「送信中…/回答/エラー」をミラー表示（**新経路を作らず Phase 1 の `askBridge` 経由**）[tdd:required]（ask 遷移 reducer/配線を `askBridge` モックで idle→thinking→answer/error 検証） | `bun test` グリーン（送信→thinking→answer / 失敗→error の遷移）。`bun run build` 成功・`biome check` 0。シミュレーターでスマホから送信→グラスに Thinking→回答、スマホにミラー表示（実機はユーザー） | 2.4 | cc:完了 [342c771]（**TDD**。「新経路を作らず Phase 1 askBridge 経由」＝ AppGlasses 内に埋め込まれていた ask ラウンドトリップを共有 `glass/ask.ts`〔`runAsk(dispatch,label,text,{ask?,isCurrent?,sessionId?})`＝`dispatch ASK`→`askBridge`→`ANSWERED`/`FAIL`、`ASK_SESSION_ID='g2-main'`/`CUSTOM_ASK_LABEL='(カスタム)'`〕へ抽出し askBridge 呼び出しを一本化。AppGlasses は録音世代 `genRef` を `isCurrent` に渡し stale guard、App は AskBox 用にそのまま使用。RED→GREEN: `ask.test.ts` 8 ケース〔実 reducer を通し 成功 idle→thinking→answer・失敗→error・pages 空→text 畳み込み・全空→`(回答がありません)`・`isCurrent`=false で結果破棄・既定 sessionId/mode=short・カスタムラベル〕。`companion/AskBox.tsx`＝`Textarea`+送信 `Button`、同一 `state` 購読で送信中/回答(pages join)/エラーを `Card` ミラー。`Companion.tsx`＝「その場で送る」`SectionHeader`+`AskBox` を先頭セクションに追加（`state`/`onAsk` プロップ）。`App.tsx`＝`handleAsk`〔`runAsk(dispatch,CUSTOM_ASK_LABEL,text)`〕を Companion へ配線。**Codex review 正規ルート**（公式 openai-codex v1.0.4 / `--base 7ab23a2 --scope branch`）初回 P2 を即修正→再レビュー APPROVE: 録音/文字起こし/review 中の AskBox 送信が共有 state へ `ASK` を割り込ませ、後続の `STOP_RECORDING`/`TRANSCRIBED` が thinking/answer を上書き・録音が最大時間まで残り得る競合 → 送信を安全 phase `READY_PHASES=[idle,answer,error]` に限定し音声フロー中はヒント表示で無効化。検証: `bun test` 84 pass・`biome check` 0・`bun run --filter g2hermes build` 成功〔CSS 32.61 kB〕・`git diff app.json` 空。シミュレーターでのスマホ送信→グラス Thinking→回答→ミラーの目視・実機はユーザー） |

...
> **実装メモ（2026-06-09・f99fdd3）**: コア 4.2/4.3/4.4 を TDD で実装・コミット済（bun test 95 pass・biome 0・build 成功・app.json 不変）。glyph は確定方針どおり `thinking`=8 方向矢印・`transcribing`=流れるドット（●幅5）を採用。グリフ収録は Codex 検証済みのため、残る **4.1（シミュレーターでの滲み目視）と 4.5（thinking 回転目視 + version bump + `evenhub pack` + 実機）はユーザー担当**。シミュレーターで矢印が滲む場合のみ `screen.ts` の `THINKING_GLYPHS` を線スピナー `│╱─╲` に差し替える（純関数 `thinkingSpinner` の定数 1 箇所変更で済む）。

...
G2 mic ──audioControl(true)──▶ apps/g2hermes (WebView, even-toolkit)
   │  GlassBridgeSource.onAudioData(pcm: Float32Array, 16000)
...
- 入力: **binary WAV body**（`Content-Type: audio/wav`）。Fastify は `addContentTypeParser` で raw Buffer を受ける（multipart 依存を足さない）。
...
- 取得経路:
  ...
  2. `source.onAudioData((pcm, rate) => buffer.append(pcm))` で Float32Array（16kHz）を蓄積

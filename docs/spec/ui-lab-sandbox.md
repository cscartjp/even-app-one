...
**目的**: 「クラスの画面（＝グラス表示）」が *絶対座標で置いたテキストコンテナの集合* であることを体で理解し、`borderWidth` / `borderRadius` / `paddingLength` / 選択表現 / レイアウト骨格 / 明るさ（擬似モーダル）を振って見比べ、気に入った数値を実アプリへ移植できるようにする。
...
| `selectionStyle` | `'inverted' \| 'cursor' \| 'filled' \| 'thickBorder'` | 選択中行の見せ方 |
| `showStatusBar` | bool | 上部ステータスバー有無 |
| `separator` | `'none' \| 'line' \| 'dots'` | 区切り線スタイル |
| `skeleton` | `'list' \| 'cards' \| 'split'` | レイアウト骨格 |
| `textColor` | int `0–15` | 本文の明るさ |
| `modal` | bool | ON で擬似モーダル（背景 dim ＋前面に明るい枠カード） |
| `modalDim` | int `0–15` | 背景の暗さ（modal=ON 時に背景コンテナ色をこの値へ落とす） |

既定値は全フィールド公式レンジ内（例: `borderWidth:2, borderRadius:7, borderColor:12, padding:8, selectionStyle:'cursor', skeleton:'cards', textColor:15, modal:false, modalDim:4`）。
...
4. `selectionStyle` の各値が選択中行に対応する見た目を出す:
   - `inverted` = 反転 / `cursor` = 行頭 `▶ `（無ちらつき）/ `filled` = 塗り背景 / `thickBorder` = その行に太枠。
5. `skeleton` 分岐: `list`（枠なし行リスト）/ `cards`（角丸枠カード群）/ `split`（左右2カラム）。
6. `modal==true` のとき: 背景コンテナの色を `modalDim` へ落とし、前面に `textColor` の明るい枠カードを1枚重ねる。
...
- 数値（borderWidth/radius/color/padding/textColor/modalDim/cardW/H/lineGap）= スライダー。

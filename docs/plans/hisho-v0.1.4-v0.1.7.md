> **アーカイブ（2026-06-08）**: 本ファイルは Hisho アプリ（`apps/hisho/`）の v0.1.4〜v0.1.7 計画の記録です。**全タスク完了済み**のためルート `Plans.md` から退避しました。Hisho の UI 正本は `apps/hisho/preview/design-mock.html`（chmod 444・変更禁止）。現行のアクティブ計画はルート `Plans.md` を参照。
